...
@Injectable()
export class FindMeteorologyAssetByInfrastructurePointIdUseCase {
  public constructor(
    @Inject(METEOROLOGY_ASSET_REPOSITORY)
    private readonly meteorologyAssetRepository: MeteorologyAssetRepository,
  ) {}

  public async execute(infrastructurePointId: number): Promise<MeteorologyAsset | null> {
    return this.meteorologyAssetRepository.findByInfrastructurePointId(infrastructurePointId);
  }
}
